...
class PieceStorage {
public:
    PieceStorage(const TorrentFile& tf, const std::filesystem::path& outputDirectory, size_t needDowland);
    ...
    void BackToQueue(const PiecePtr& piece);
...
private:
    ...
    std::vector<size_t> savedToDisk_;
    std::ofstream file_;
    size_t pieceLength_, piecesCount_;
